...
export class StoryService {
  ...
  constructor(
    ...
    this.host = opts.host ?? new WorkspaceHost(project.root, project.backlogDirectory, project.storyMapsDirectory);
  ...
  private plan(
    ...
    const reparsed = parseWorkItem(next, item.sourcePath, item.completed).item;
  ...
  private readCurrent(id: string): { ws: Workspace; item: WorkItem; path: string; raw: string } {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      const item = parseWorkItem(raw, cached.sourcePath, cached.completed).item;
  ...
  create(body: unknown, opts: { dryRun?: boolean } = {}): { dryRun: boolean; story: ApiStory } {
    ...
    const run = () => {
      ...
      const parsed = parseWorkItem(content, relPath, false);
