...
function toQuery(filter: StoryFilter | StoryQuery): StoryQuery {
  return 'fields' in filter && typeof filter.fields === 'object' ? (filter as StoryQuery) : storyFilterToQuery(filter as StoryFilter);
}
...
export function matches(
  ...
  return matchesStoryQuery(item, toQuery(filter), mapMembership);
}
